...
- **The discovery pipeline (admin → Discovery) is intentionally conservative.** "Run Scan" checks that an approved source URL is still reachable and logs the result — it does not scrape or fabricate opportunities, since there's no content-extraction worker wired up yet. See the schema for `ExtractedOpportunityData` in `src/lib/types.ts`, which is the contract a future extraction step should fill in; discovered items always land in an admin review queue and are never published automatically.
